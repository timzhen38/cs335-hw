...
//internal quicksort using middle element as pivot
template <typename Comparable, typename Comparator>
...
//internal quicksort using first element as pivot
template <typename Comparable, typename Comparator>
void quicksort3( vector<Comparable> & a, int left, int right, Comparator less_than )
{
    if( left + 10 <= right )
    {
        const Comparable & pivot = first( a, left, right, less_than );
